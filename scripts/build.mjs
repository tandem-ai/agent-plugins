...
for (const [manifest, fields] of [
  ...
  for (const field of fields) {
    const asset = manifest === ".codex-plugin/plugin.json" ? data.interface?.[field] : data[field];
...
for (const [env, hosts] of Object.entries(ENVS)) {
  ...
  if (check) {
    ...
    for (const asset of assets) {
      ...
      if (!statSync(target, { throwIfNoEntry: false })?.isFile() || !readFileSync(target).equals(readFileSync(join(root, asset)))) problems.push(`variants/${env}/${asset} is stale, run npm run build`);
    }
    continue;
  }
  for (const f of VARIANT_FILES) {
  ...
  for (const asset of assets) {
    mkdirSync(join(out, asset, ".."), { recursive: true });
    cpSync(join(root, asset), join(out, asset));
  }
