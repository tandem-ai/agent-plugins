...
for (const [env, hosts] of Object.entries(ENVS)) {
  ...
  if (check) {
    ...
    const icon = join(out, "assets/icon.png");
    if (!existsSync(icon) || !readFileSync(icon).equals(readFileSync(join(root, "assets/icon.png")))) problems.push(`variants/${env}/assets/icon.png is stale, run npm run build`);
    continue;
  }
  for (const f of VARIANT_FILES) {
  ...
  cpSync(join(root, "assets/icon.png"), join(out, "assets/icon.png"));
